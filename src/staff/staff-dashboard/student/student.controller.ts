...
export class StudentController {
  ...
  @Get('my-students')
  @ApiOperation({
    summary: 'Get students assigned to the current staff member',
  })
  @ApiOkResponse({
    description: 'Students returned successfully',
    type: StudentResponse,
  ...
  async findMyStudents(
    ...
    const { data, total } = await this.studentService.findMyStudents(
      userId,
      query,
    );
    const limit = query.limit || 10;
  ...
  @UseInterceptors(NoFilesInterceptor())
  @ApiOperation({
    summary: 'Assign a staff member (sales/support) to a student',
  })
  @ApiParam({ name: 'id', type: Number })
  @ApiOkResponse({
