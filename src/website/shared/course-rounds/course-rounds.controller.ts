...
export class GuestCourseRoundsController {
  ...
  @ApiOperation({
    summary:
      'List all course rounds with course and trainer relations without pagination',
  })
  @ApiQuery({
    name: 'lang',
    required: false,
  ...
  @ApiOperation({
    summary: 'Get course round details by course slug and round Id',
  })
  @ApiParam({
    name: 'courseSlug',
    type: String,
  ...
  async findOne(
    ...
    const lang = getLang();
    const round = await this.courseRoundsService.findOneBySlugAndRoundNumber(
      courseSlug,
      roundId,
    );
