...
export class CoursesController {
    ...
    async create(
        ...
        @UploadedFiles() files: { image?: Express.Multer.File[], introVideo?: Express.Multer.File[] }
    ): Promise<ApiResponseDto<CourseResponse>> {
        let image: string | undefined;
        let introVideo: string | undefined;

        if (files?.image?.[0]) {
            const uploaded = await this.storageService.upload(files.image[0], UploadType.IMAGE);
            image = uploaded.url;
        }
        if (files?.introVideo?.[0]) {
            const uploaded = await this.storageService.upload(files.introVideo[0], UploadType.VIDEO);
            introVideo = uploaded.url;
        }
        const course = await this.coursesService.create(dto, image, introVideo);
    ...
    @Get(':id')
    @ApiOperation({ summary: 'Return a single course with its trainer and category' })
    @ApiParam({ name: 'id', type: Number })
    @ApiResponse({ status: 200, type: CourseResponse })
    async findOne(@Param('id', ParseIntPipe) id: number): Promise<ApiResponseDto<CourseResponse>> {
    ...
    async update(
        ...
        const course = await this.coursesService.update(id, dto, image, introVideo);
        return ApiResponseDto.success(course);
    }
